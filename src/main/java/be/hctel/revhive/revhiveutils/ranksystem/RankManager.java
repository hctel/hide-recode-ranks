...
public class RankManager {
	...
	public Rank getRank(Player player) {
		if(cache.containsKey(player)) {
			...
			int rankID = connection.getInt("ranks", uuid, "rankID");
			switch (rankID) {
			case 0:
	...
	public boolean rankHidden(Player player) {
		if(cacheHidden.containsKey(player)) {
			...
			String uuid = getUUID(player);
			int hid = connection.getInt("ranks", uuid, "hiddenRank");
			switch(hid) {
			case 0:
	...
	public void setRank(Player player, Rank rank) {
		...
		connection.insert("ranks", uuid, "rankID", rankID);
	}
	/**
	...
	public void setRankHidden(Player player, boolean hidden) {
		...
		connection.insert("ranks", uuid, "hiddenRank", sql);
	}
	/**
	...
	/**
	 * Use when a player joins
	 * @param player the player involved in the PlayerJoinEvent
	 */
	public void join(Player player) {
		if(!connection.checkIfPlayerExists("ranks", getUUID(player))) {
			connection.insert("ranks", getUUID(player), "rankID", 0);
			connection.insert("ranks", getUUID(player), "hiddenRank", 0);
		}
	}
